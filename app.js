...
let countryCode1 = "USD";
const getCountryCode1 = () => {
  const country = document.querySelector("#selectCountry1");
  countryCode1 = country.value;
...
async function getExchangeRate(toCcode,fromCcode) {
  ...
  const tocRate = countryAndRate[toCcode] || "1" ;
  const fromcRate = countryAndRate[fromCcode] || "1" ;

  // document.querySelector('[ex-result]').innerHTML =`The exchange rate is: ${tocRate} ${countryCode}.`;
  console.log(toCcode);
  ...
  console.log(tocRate);
    ...
    let exchangeResult = ( Number(inputAmountEle.value) / Number(fromcRate) )  * Number(tocRate);
...
const inputAmountEle = document.querySelector('[inputAmount]');
  ...
  calExBtn.onclick = () =>  {
    calculateExchange(inputAmountEle , tocRate);
...
getExchangeRate(countryCode , countryCode1);
